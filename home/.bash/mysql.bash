...
#    version=${version%%-*}
    ...
    local my_base_port=${my_base_port:-${version}0}
...
my_client() {
    ...
    shift
    eval "$(
        echo ./bin/mysql --defaults-file=\<\(my_cnf $id\) 
    ) $(
        (($#)) && printf '%q ' "$@"
    )"
}
...
my_pid () { if ! (($#)); then printf 'PID required. Aborting.\n'>&2; return 1; fi; while read -r ln; do [[ $ln = n* ]] && dir="${ln:1}"; done < <(lsof -d cwd -a -p $1 -Fn) && cd "$dir/../" && "./bin/mysql" -S "$dir/mysql.sock"; }
